...
describe('Alert.vue', () => {
	...
	it('should support centering text', () => {
		...
		//class
		const rootNode = wrapper.find('.vr-alert')
	...
	it('should not render close icon when closable is false', () => {
		...
		expect(wrapper.find('.vr-alert__close').exists()).toBe(false)
	})
	...
	it('should toggle visibility when open and close methods are called', async () => {
		...
		expect(wrapper.find('.vr-alert').attributes().style).toBe('display: none;')
		await wrapper.vm.open()
		expect(wrapper.find('.vr-alert').attributes().style).toBe('')
	})
})
